...
async function getAllCommitMessages(octokitClient, prMetadata) {
  ...
  for (let c of commitResponse.data) {
    ...
    messages.push(
      ...(await searchForCommitPullRequest(gitCacheClient, prMetadata, c))
    )
  }
...
async function searchForCommitPullRequest(
  searchClient,
  prMetadata,
  commitMetadata
) {
  ...
  console.log(
    `Request search PR for sha ${commitMetadata.sha}:  ${searchResponse.headers['x-cache']}`
  )
  let prBodies = []
  if (searchResponse.data.total_count > 1) {
    searchResponse.data.items.forEach((i) => {
      if (
        i.pull_request.url !==
        `https://api.github.com/repos/${prMetadata.owner}/${prMetadata.repo}/pulls/${prMetadata.prNumber}`
      ) {
...
async function run(owner, repository, prNumber) {
  const prMetadata = {
    owner,
    repository,
    prNumber
