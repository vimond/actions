...
async function run() {
  try {
    ...
    let textBlocks = await prMetadataCollector.getAllTextBlocks(
      input.owner,
      input.repo,
      input.prNumber
    )
    ...
    let filteredTickets
    if (
      jiraConfig.host !== undefined &&
      jiraConfig.token !== undefined &&
      jiraConfig.username !== undefined
    ) {
      try {
        filteredTickets = await jira.checkIfExist(
          nodeFetch,
          jiraConfig,
          ticketsFound
        )
    ...
    core.setOutput(
      'tickets',
      Buffer.from(JSON.stringify(filteredTickets)).toString('base64')
    )
